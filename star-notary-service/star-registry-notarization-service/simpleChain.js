...
class Blockchain{
 ...
 getBlocksByAddress(address){
  // return promise
  return new Promise((resolve, reject) => getBlocksByAddressFromLevelDB(address, function(error, value) {
    if (error) {
         console.log(error);
         resolve('No blocks found for '+address);
       } else {
          console.log('blocks by', address, 'retrieved from database');
          resolve(value);
      }
    }
    ));
...
getBlockByHash(hash){
  // return promise
  return new Promise((resolve, reject) => getBlockByHashFromLevelDB(hash, function(error, value) {
    if (error) {
         console.log(error);
         resolve('No blocks found for '+address);
       } else {
          console.log('block by', hash, 'retrieved from database');
          resolve(value);
      }
    }
    ));
...
function getBlocksByAddressFromLevelDB(address, callback) {
  let blocks = [];
  db.createReadStream().on('data', function (data) {
...
function getBlockByHashFromLevelDB(hash, callback) {
  let blocks = [];
  db.createReadStream().on('data', function (data) {
    let obj = JSON.parse(data.value);
      if (obj.hash === hash) {
